...
const AdminNurturingManagement = ({ isOpen, onClose }) => {
  ...
  const fetchNurturing = async () => {
    setLoading(true);
    try {
      const response = await axios.get('/nurturing');

      if (response.data.success) {
        setNurturing(response.data.data);
        setStats(response.data.stats);
      }
    } catch (error) {
      console.error('Error fetching nurturing records:', error);
      toast.error('Failed to load nurturing records');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (id) => {
    try {
      const response = await axios.put(`/nurturing/${id}/approve`);

      if (response.data.success) {
        toast.success('Nurturing record approved!');
        fetchNurturing(); // Refresh the list
      }
    } catch (error) {
      console.error('Error approving nurturing:', error);
      toast.error(error.response?.data?.message || 'Failed to approve nurturing');
    }
  };

  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this nurturing record?')) {
      return;
    }

    try {
      const response = await axios.delete(`/nurturing/${id}`);

      if (response.data.success) {
        toast.success('Nurturing record deleted!');
        fetchNurturing(); // Refresh the list
      }
    } catch (error) {
      console.error('Error deleting nurturing:', error);
      toast.error(error.response?.data?.message || 'Failed to delete nurturing');
    }
  };
  ...
  const filteredNurturing = filterStatus === 'all'
    ? nurturing
    : nurturing.filter(n => n.status === filterStatus);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-orange-600 via-orange-700 to-red-700 text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <FiHeart className="w-6 h-6" />
            <div>
              <h2 className="text-xl md:text-2xl font-bold">Nurturing Management</h2>
              <p className="text-sm text-orange-100">Super Admin Panel</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex gap-2 p-4 border-b bg-white overflow-x-auto">
          {['all', 'Pending', 'Approved', 'In Progress', 'Rejected'].map(status => (
            <button
              key={status}
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-320px)] p-4">
          {loading ? (
            <div className="text-center py-8">
              <p className="text-gray-600">Loading nurturing records...</p>
            </div>
          ) : filteredNurturing.length === 0 ? (
            <div className="text-center py-8">
              <FiHeart className="w-12 h-12 text-gray-400 mx-auto mb-2" />
              <p className="text-gray-600">No nurturing records found</p>
            </div>
          ) : (
            <div className="space-y-3">
              {filteredNurturing.map((record) => (
                <div
                  key={record._id}
                      ...
                      <div className="flex items-start gap-3">
                        <div className="flex-1">
                          ...
                          <div className="mt-2 text-sm">
                            <span className="text-gray-600">Nurtured by: </span>
                            <span className="font-semibold text-orange-700">
                              {record.nurturedBy
                                ? `${record.nurturedBy.firstName} ${record.nurturedBy.lastName}`
                                : 'Unknown'}
                            </span>
                          </div>
                          {record.sessions && record.sessions.length > 0 && (
                            <div className="mt-1 text-xs text-gray-500">
                              {record.sessions.length} session{record.sessions.length !== 1 ? 's' : ''} recorded
                            </div>
                          )}
                        </div>
                      </div>
...
export default AdminNurturingManagement;
