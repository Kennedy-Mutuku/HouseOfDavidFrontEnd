...
const AddNurturingModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.fullName || !formData.phone || !formData.startDate) {
      toast.error('Please enter name, phone number and start date');
      return;
    }
    ...
    try {
      ...
      const message = error.response?.data?.message || 'Failed to add nurturing record';
      toast.error(message);
    } finally {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <p className="text-gray-600 text-sm">
            Record members you are nurturing
          </p>
...
export default AddNurturingModal;
